...
const GComunicaciones = () => {

  const [registros, setRegistros] = useState([]);
  const obtenerRegistros = async () => {
    try {
      const response = await axios.get('http://localhost:3001/api/MostrarDatosTCN');
    ...
    <div>
      <div style={{ width: '100%' }}>
        <header className="custom-header">
          <h1>INTRANET-DTDS</h1>
          <h1 style={{ marginLeft: '400px' }}>Servicio de Gestion de info de solicitudes</h1>
      ...
      <div className="caja-datos" style={{ marginTop: '-50px', width: '100%' }}>
        <h2>Informacion de los que desean trabajar con nosotros</h2><br/>
          ...
          <tbody>
            {registros.map((registro) => (
              <tr key={registro._id}>
                ...
                <td>{registro.pais}</td>
                <td>{registro.cuidad}</td>
                <td>{registro.cpostal}</td>
...
export default GComunicaciones;
